...
public class VMRelation {
    public static void updateDiskRelation (VirtualMachine vm) throws JSONException, IOException {
        HttpUtil.deleteRelation(RelationshipMapping.VM_FOR_DISK.split("Map_")[1],"Disk","name",vm.getDiskName(),"VirtualMachine","name",vm.getName());
    ...
    public static void updateInstanceGroupRelation (VirtualMachine vm) throws JSONException, IOException {
        HttpUtil.deleteRelation(RelationshipMapping.VM_IG_RELATION.split("Map_")[1],"InstanceGroup","name", !Objects.equals(vm.getInstanceGroupName(), "") ? vm.getInstanceGroupName().split("instanceGroups/")[1]:"","VirtualMachine","name",vm.getName());
    ...
    public static void updateSubnetRelation (VirtualMachine vm) throws JSONException, IOException {
        HttpUtil.deleteRelation(RelationshipMapping.SUBNET_FOR_VM.split("Map_")[1],"Subnet","name", (vm.getSubnet() != null && !vm.getSubnet().isEmpty()) ? vm.getSubnet().split("regions/")[1]:"","VirtualMachine","name",vm.getName());
